...
public class Sphere extends Object3D {
    private Vector3D center;  // Center point of the sphere
    ...
    public Intersection intersectAnalytic(Ray ray) {
        Vector3D oc = Vector3D.subtract(center, ray.getOrigin());
        ...
        if (discriminant < 0) {
            ...
            if (t > 0) {
                Vector3D hitPoint = ray.getOrigin().add(ray.getDirection().scale(t));
                return new Intersection(hitPoint, t, this.getColor());
            }
        }
        return null;
    }
    ...
    public Intersection[] intersect(Ray ray) {
        ...
        Vector3D L = Vector3D.subtract(this.getCenter(),O);
        double tCA = L.dot(D);
        ...
        if(tCA < 0) return null;  // Sphere is behind ray origin
            ...
            if(d < 0) return null;  // No intersection
                ...
                Intersection p0 = new Intersection(O.add(D.scale(t0)), t0, this.getColor());
                Intersection p1 = new Intersection(O.add(D.scale(t1)), t1, this.getColor());
