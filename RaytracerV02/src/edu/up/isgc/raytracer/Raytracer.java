...
public class Raytracer {
    public static void main(String[] args) {
        ...
        int width = 400;
        int height = 400;
        Double nearPlane = 4.4, farPlane = 10.0;

        // Create scene with objects
        Scene scene = new Scene();
        scene.addObject(new Sphere(new Vector3D(-0.5, 0, 5), 0.3, Color.RED));
        scene.addObject(new Sphere(new Vector3D(0.4, 0, 4.5), 0.2, Color.BLUE));

        // Set up camera at the origin
        Camera camera = new Camera(new Vector3D(0, 0, 0), nearPlane, farPlane);
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                Intersection intersection = scene.findClosestIntersection(ray, camera.getClipPlanes());
